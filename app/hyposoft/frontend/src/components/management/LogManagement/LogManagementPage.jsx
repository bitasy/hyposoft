...
async function createLogArray(username, displayName, identifier, model, page) {
    const { results, count } = await RealAPI.getLog(username, displayName, model, identifier, page);
...
class LogManagementPage extends React.Component {
    ...
    componentDidMount() {
        createLogArray(this.state.username, this.state.displayName, this.state.identifier, this.state.model, this.state.page)
            .then(({data, total}) => this.setState({datasource: data, total }));
    }

    componentDidUpdate(prevProps, prevState, snapshot) {
        console.log(this.state.page);
        if(!(this.state.page === prevState.page && prevState.username === this.state.username && prevState.displayName === this.state.displayName && prevState.identifier === this.state.identifier && prevState.model === this.state.model)) {
            if(this.state.page === prevState.page) {
                createLogArray(this.state.username, this.state.displayName, this.state.identifier, this.state.model, 1)
                    .then(({data, total}) => this.setState({datasource: data, total, page: 1 }));
            } else {
                createLogArray(this.state.username, this.state.displayName, this.state.identifier, this.state.model, this.state.page)
                    .then(({data, total}) => this.setState({datasource: data, total }));
            }
    ...
    render() {
        return (
            <div style={{ padding: 16 }}>
                ...
                <Pagination page={this.state.page} total={this.state.total} pageSize={10} onChange={(page, pageSize) => {
                    this.setState({ page });
                }}/>
            </div>
        );
