...
class AssetPermissionCreateMixin(CreateAndLogMixin):
    def perform_create(self, serializer):
        abbr = self.get_object().datacenter.abbr
        if self.request.user.is_superuser:
            super(AssetPermissionCreateMixin, self).perform_create(serializer)
            return
        else:
            data_perm = self.request.user.permission.datacenter_perm
            if not self.request.user.permission.asset_perm or ('Global' not in data_perm and abbr not in data_perm):
                raise serializers.ValidationError("You don't have permission.")
        super(AssetPermissionCreateMixin, self).perform_create(serializer)
